const allQuestions = [
  {
    "id": 1,
    "type": "mc",
    "question": "An administrator accidentally deleted the /boot/vmlinuz file an d must resolve the issue before the server is rebooted. Which of the following commands should the administrator use to identify the correct version of this file?",
    "choices": [
      "rp m -qa | grep kernel; uname -a",
      "yum -y update; shutdown -r no w",
      "cat /etc/centos-release; rpm -Uvh --nodeps",
      "telinit 1 ; restorecon -Rv /boo t Answ er: A Explanation: The command rpm -qa | grep kernel lists all the installed kernel packages, and the command uname -a displays the current kernel version. These commands can help the administrator identify the c orrect version of the /boot/vmlinuz file, which is the kernel image file. The other options are not relevant or helpful for this task. Reference: CompTIA Linux+ (X K0-005) Certification Study Guide, Chapter 8: Managing the Linux Boot Process, page 267."
    ],
    "answer": "A",
    "explanation": ""
  },
  ...
  },
  {
    "id": 6,
    "type": "mc",
    "question": "A user is asking the systems adm inistrator for assistance with writing a script to verify whether a file exists. Given the following: Which of th e following commands should replace the <CONDITIONAL > string?",
    "choices": [
      "if [ -f \"$filename\" ]; then",
      "i f [ -d \"$filename\" ]; then",
      "i f [ -f \"$filename\" ] then",
      "if [ -f \"$filename\" ]; whi le Answ er: A Explanation: The command if [ -f \"$filename\" ]; then checks if the variable $filename refers to a regular file that exists. The -f option is used to test for files. If the conditi on is true, the commands after then are executed. This is the correct w ay to replace the <CONDITIONAL> string. The other options are incorrect because they either use the wrong option (-d tests for directories), the wrong syntax (missing a semicolon after the condition), or the wrong keyword (while is used for loops, not conditions). Reference: CompTIA Linux+ (XK0-005) Certification Study Guide, Chapter 16: Writing and Executing Bash Shell Scripts, page 493."
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    "id": 8,
    "type": "mc",
    "question": "A systems administrator is deploying three identical, cloud-bas ed servers. The administrator is using the following code to complete the task: Which of th e following technologies is the administrator using?",
    "choices": [
      "An sible",
      "Puppet",
      "Ch ef",
      "Terra form Answ er: D Explanation: The code snippet is written in Terraform language, which is a t ool for building, changing, and versioning infrastructure as code. Terraform uses a declarative syntax to describe the desired state of the infrastructure and applies the changes accordingly. The code defines a resource of type aws_instance, which creates an AWS EC2 instance, and sets the attributes such as the AMI ID, instance type, security group IDs, and key name. The code also uses a count parameter to create three identical instances and assigns them different names usin g the count.index variable. This is the correct technology that the administrator is using. The other o ptions are incorrect because they use different languages and syntaxes for infrastructure as code. Re ference: CompTIA Linux+ (XK0-005) Certification Study Guide, Chapt er 19: Managing Cloud and Virtu alization Technologies, page 559."
    ],
    "answer": "A",
    "explanation": ""
  },
  ...
  },
  {
    ...
    "choices": [
      "tar -cvz f /dev/sdd1 /dev/sdc1",
      "rs ync /dev/sdc1 /dev/ sdd1",
      "dd if=/d ev/sdc1 of=/dev/sdd1",
      "scp /dev /sdc1 /dev/sd d1 Answ er: C Explanation: The command dd if=/dev/sdc1 of=/dev/sdd1 copies the data from the input file (if) /dev/sdc1 to the output file (of) /dev/sdd1, byte by byte. This is the correct w ay to clone a partition. The other options are incorrect because they either compress the data (tar -cvzf) , synchronize the files (rsync), or copy the files over a network (scp), which are not the same as cloni ng a partition. Reference: CompTIA Linux+ (XK0-005) Certification Stud y Guide, Chapter 10: Managing Storage, page 321."
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    ...
    "choices": [
      ...
      "ssh-ke ygen",
      "ss h-keyscan",
      "ssh-cop y-id E. ftpd F. scp Answ er: D, F Explanation: The commands ssh-copy-id and scp can be used to copy a key file to remote servers. The command ssh-copy-id copies the public key to the authorized_key s file on the remote server, which allows the user to log in without a password. The command scp c opies files securely over SSH, which can be used to transfer the key f ile to any location on the rem ote server. The other options are incorrect because they are not related to copying key files. Th e command wget downloads files from the web, the command ssh-keygen generates key pairs, the comman d ssh-keyscan collects public keys from remote hosts, and the command ftpd is a FTP server da emon. Reference: CompTIA Linux+ (XK0-005) Certification Study Guide, Chapter 13: Managing Netwo rk Services, pages 408-410."
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    ...
    "choices": [
      ...
      "s ysctl -w net.ipv4.ip_forward=1",
      "fire wall-cmd --enable ipv4_forwarding",
      "s ystemct1 start ipv4_ forwarding Answ er: B Explanation: The command sysctl -w net.ipv4.ip_forward=1 enables IPv4 packet forwarding temporarily by setting the kernel parameter net.ipv4.ip_forward to 1. To make this cha nge persistent, the administrator needs to edit the file /etc/sysctl.conf and add the line net.ip v4.ip_forward = 1. The other options are incorrect because they either use the wrong file (/proc/sys/net/ipv4/ipv_forward), the wrong"
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    "id": 20,
    "type": "mc",
    "question": "A systems administrator wants to back up the directory /data an d all its contents to /backup/data on a remote server named remote. Which of the following commands w ill achieve the desired effect?",
    "choices": [
      "scp -p /data r emote:/backup/da ta",
      "ssh -i /remote:/backup/ /da ta",
      "rsy nc -a /data remote:/backup/",
      "cp -r /data /r emote/backu p/ Answ er: C Explanation: The command that will back up the directory /data and all its c ontents to /backup/data on a remote server named remote is rsync -a /data remote:/backup/. This com mand uses the rsync tool, which is a remote and local file synchronization tool. It uses an algori thm to minimize the amount of data copied by only moving the portions of files that have changed. The -a option stands for archive mode, which preserves the permissions, ownership, timestamps, and sym bolic links of the files. The /data argument specifies the source directory to be backed up, and th e remote:/backup/ argument specifies the destination direct ory on the remote server. The rsync tool will create a subdirectory named data under /backup/ on the remote server, and copy all th e files and subdirectories from /data on the local server. The other options are not correct commands for backing up a dir ectory to a remote server. The scp -p /data remote:/backup/data command will copy the /data directory as a file named data under /backup/ on the remote server, not as a subdirectory with its c ontents. The -p option preserves the"
    ],
    "answer": "A",
    "explanation": ""
  },
  ...
  },
  {
    "id": 34,
    "type": "mc",
    "question": "A Linux administrator has instal led a web server, a database se rver, and a web application on a server. The web application should be active in order to render the web pages. After the administrator restarts the serve r, the website displays the fol lowing message in the browser: Error establishing a database connection. The Linux administrator reviews the following relevant output from the systemd init files: The administrator needs to ensure that the database is available before the web application is started. Which of the following should the administrator add to the HTTP server .service file to accomplish this task?",
    "choices": [
      "TRIGG ERS=mariadb.servi ce",
      "ONFAI LURE=mariadb.serv ice",
      "WANTEDBY=m ariadb.service",
      "REQUIRES= mariadb.servi ce Answ er: D Explanation: The administrator should add REQUIRES=mariadb.service to the HT TP server .service file to ensure that the database is available before the web application is st arted. This directive specifies that the"
    ],
    "answer": "A",
    "explanation": ""
  },
  ...
  },
  {
    "id": 92,
    "type": "mc",
    "question": "A systems administrator is tasked with installing GRUB on the l egacy MBR of the SATA hard drive. Which of the following commands will help the administrator acc omplish this task?",
    "choices": [
      "grub-ins tall /dev/hda",
      "grub-inst all /dev /sda",
      "grub-inst all /dev /sr0",
      "grub-inst all /dev/hd0, 0 Answ er: B Explanation: The command that will help the administrator install GRUB on th e legacy MBR of the SATA hard drive is grub-install /dev/sda. This command will install GRUB on the master boot record (MBR) of the first SATA disk (/dev/sda). The MBR is t he first sector of a disk tha t contains boot code and a partition table. GRUB will overwrite the boot code and place its own code that can load GRUB modules and configuration files from a specific partition. The other options are not correct commands for installing GRUB on the legacy MBR of the SATA hard"
    ],
    "answer": "A",
    "explanation": ""
  },
  ...
  },
  {
    "id": 200,
    "type": "mc",
    "question": "Due to performance issues on a server, a Linux administrator ne eds to termi-nate an unresponsive process. Which of the following commands should the administrator use to terminate the process immediately without waiting for a graceful shutdown?",
    "choices": [
      "kill -SIGKILL 5545",
      "kil l -SIGTERM 5545",
      "kil l -SIGHUP 5545",
      "kill -SIGINT 5545 Answ er: A Explanation: To terminate an unresponsive process immediately without waitin g for a graceful shutdown, the administrator can use the command kill -SIGKILL 5545 (A). This will send a signal to the process with the PID 5545 that cannot be ignored or handled by the process, and force it to stop. The other commands will send different signals that may allow the process to perform some cleanup or termination actions, or may be ignored by the process. Reference: [CompTIA Linux+ Study Guide], Chapter 6: Managing Processes, Se ction: Killing Processes [How to Kill Processes in Linux]"
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    ...
    "choices": [
      ...
      "blk id",
      "d msetup",
      "Isscsi Answ er: B Explanation: To obtain information about the UUIDs of all disks attached to a Linux system, the administrator can run the command blkid (B). This will display the block device a ttributes, including the UUID, label, type, and partition information. The other commands are not related to this task. Reference: [CompTIA Linux+ Study Guide], Chapter 5: Managing Filesystems a nd Logical Volumes, Section: Identifying Disks by UUID [How to Use blkid Command in Linux]"
    ],
    "answer": "A",
    "explanation": ""
  },
  ...
  },
  {
    "id": 203,
    "type": "mc",
    "question": "An administrator would like to mirror the website files on the primary web server, www1, to the backup web server, www2. Which of the following commands should the administrator use to most efficiently accomplish thi s task?",
    "choices": [
      "[w wwl ] rsync \"a \"e ssh /var/ www/html/ : /var/www /html",
      "[ ww wl ] scp \"r /var/www/html : / var/www/htm l",
      "[ww w2 ] cd /var/www/html; wget \"m http: //w wwl/",
      "[w wwl ] cd /var/www/html && tar c vf \" Answ er: A"
    ],
    "answer": "A",
    "explanation": ""
  },
  ...
  },
  {
    "id": 259,
    "type": "mc",
    "question": "After connecting to a remote host via SSH, an administrator attempts to run an application but receives the following error: [ ~]$ ssh Last login: Tue Mar 29 18:03:34 2022 [ ~] $ /usr/local/bin/config_manager Error: cannot open display: [ ~] $ Which of the following should the administrator do to resolve t his error?",
    "choices": [
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "The mount unit file name and the Where entry must be escaped to  handle spaces in the path.\nReference\nThe mount unit file name must be named after the mount point di rectory, with spaces replaced\nby \\x20. See How to escape spaces in systemd unit files? and sy stemd.mount.\nThe Where entry must use \\x20 to escape spaces in the path. See  systemd.mount and The\nworkaround is to use /usr/bin/env followed by the path in quote s.."
  },
  {
    "id": 279,
    "type": "mc",
    "question": "A systems administrator needs to remove a disk from a Linux ser ver. The disk size is 500G, and it is the only one that size on that machine. Which of the following commands can the administrator use to find the corresponding device name?",
    "choices": [
      "fdisk -V",
      "part probe -a",
      "l susb - t",
      "lssc si -s Answ er: D Explanation: The lsscsi command can list the SCSI devices on the system, alo ng with their size and device name. The -s option shows the size of each device. The administrator can look for the device that has a size of 500G and note its device name. See lsscsi(8) - Linux man pag e and How to check Disk Interface Types in Linux. Reference 1: https://linux.die.net/man/lsscsi 2: https://www.golinuxcloud.com/check-disk-type-linux/"
    ],
    "answer": "A",
    "explanation": ""
  },
  ...
  },
  {
    "id": 285,
    "type": "mc",
    "question": "An administrator is running a web server in a container named web, but none of the error output is not showing. Which of the following should the administrator us e to generate the errors on the container?",
    "choices": [
      "docker-co mpose inspect WEB",
      "dock er logs WEB",
      "dock er run \"name WEB \"volume/dev/stdout:/var/log/nginx/error .log",
      "docker ps WEB -f Answ er: B Explanation: The docker logs command is used to fetch the logs of a containe r. If the error output is not showing for a running container, the docker logs command can be used to view these details56. Reference: 5(https://www.docker.com/blog/how-to-fix-and-debug-docker-conta iners-like-a-superhero/) 6(https://stackoverflow.com/questions83385/getting-console-outp ut-from-a-docker-container)"
    ],
    "answer": "A",
    "explanation": ""
  },
  ...
  },
  {
    ...
    "choices": [
      ...
      "/ home",
      "/ root",
      "/etc Answ er: D Explanation: The /etc directory contains confi guration files for system-wide settings and services in Linux. Files like"
    ],
    "answer": "A",
    "explanation": ""
  },
  ...
  },
  {
    "id": 324,
    "type": "mc",
    "question": "A Linux administrator encounters the following error in the Apa che log files: css SQLSTATE[HY000] [2002] Permission denied Which of the following commands should the administrator run to safely fix this issue?",
    "choices": [
      "chcon -c httpd_sys_content_t /home/apache",
      "setseboo l -F httpd_can_network_connect_db o n",
      "se tenforce 0",
      "touch /. autorelabe l Answ er: B Explanation: The error indicates that SELinux is preventing Apache from connecting to the database. The command setsebool -F httpd_can_network_connect_db on allows the Apache web server to make network connections to the database, resolving the permission denied error without disabling SELinux entirely."
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    ...
    "choices": [
      "Creat ing the /etc/nologin fil e",
      "Crea ting the /etc/nologin.allow file containing only a singl e line root",
      "Crea ting the /etc/nologin/login.deny file containing a singl e line -all",
      "Ensuring that /etc/ pam.d/sshd includes account sufficient pam_nologin .so Answ er: A Explanation: The presence of the /etc/nologin file prevents non-root users from logging into the system via SSH or any other login method. Only the root user can log in when this file exists. This file is commonly used to temporarily disable user logins during system maintenance."
    ],
    "answer": "A",
    "explanation": ""
  },
  ...
  },
  {
    "id": 329,
    "type": "mc",
    "question": "Which of the following commands should a technician use to crea te an administrative account for the username Joe?",
    "choices": [
      "su do useradd -G wheel joe",
      "sudo u seradd joe; sudo passw d -l joe",
      "sudo u seradd joe; sudo cat key.pem > ~/.ssh/authorized_keys",
      "su do useradd joe; groupadd admi n joe Answ er: A Explanation: The -G wheel option adds the user joe to the wheel group, which is a special group that allows users to execute administrative commands using sudo. This is the corr ect way to create a user with administrative privileges in many Linux distributions, includin g Red Hat-based ones."
    ],
    "answer": "A",
    "explanation": ""
  },
  ...
  },
  {
    "id": 332,
    "type": "mc",
    "question": "A Linux administrator deployed a large-scale application servic e as a containerized pod. The Linux administrator needs to continually send the application's log f ile to the companys central log store. Which of the following should the Linux administrator do to eff iciently perform this task?",
    "choices": [
      "Configure a sidecar to perform log shipping",
      "Run do cker exec to access the logs within the containe r",
      "Execu te docker inspect and run a log rsync replication",
      "Set up a custom task scheduler via kub ectl Answ er: A Explanation: In Kubernetes, a sidecar container can be added to the same pod as the main application. The sidecar handles tasks like log shipping, monitoring, or updatin g without interrupting the primary application. This is an efficient and widely-used pattern for managing application logs in containerized environments."
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    "id": 333,
    "type": "mc",
    "question": "A Linux administrator needs to run a web application on a serve r that requires the use of a desktop web browser to enter the final configuration information. The administrator determines the server is running in multi-user mode. The administrator verifies that a web browser is installed on the server and that it has a display manager installed. Which of the follo wing commands will best allow the administrator to temporarily use the web browser on the server?",
    "choices": [
      "su do systemctl isolate graphical. target",
      "sudo sy stemctl set-default runlev el5.target",
      "sudo sy stemctl restart gdm",
      "su do systemctl --no-wall rescu e Answ er: A Explanation: The systemctl isolate graphical.target command switches the sys tem to graphical mode (runlevel 5). This will start the display manager and allow the administrator to use a graphical environment, including a web browser. The swi tch is temporary, and the syste m will revert to multi-user mode after reboot unless explicitly changed."
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    "id": 334,
    "type": "mc",
    "question": "A security analyst is monitoring the network to identify latenc y or slowdowns during a vulnerability scan. Which of the following functions will best achieve this? bash function x() { info=$(ping -c 1 $1 | awk -F \"/\" 'END {print $5}') echo \"$1 | $info\" }",
    "choices": [
      "function x() { info=$(dig $(dig -x $1 | grep ptr | tail -n 1 | awk -F \".in-addr\" '{prin t $1}').origin. asn.cymru.com TXT +short); echo \"$1 | $info\" }",
      "function x() { info=$(ping -c 1 $1 | awk -F \"/\" 'END {print $5}'); echo \"$1 | $info\" }",
      "function x() { info=$(nc -m 40 $1 | awk 'END {print $1}'); e cho \"$1 | $info\" }",
      "function x() { info=$(geoiplookup $1); echo \"$1 | $info\" } Answ er: B Explanation: The ping command is used to measure network latency. The functi on provided uses ping -c 1 to ping the target once and extracts the average round-trip time using awk. This is a simple and effective way to monitor network latency during a scan or other network activ ity."
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    "id": 335,
    "type": "mc",
    "question": "Joe, a user, is unable to log in to the system. The Linux admin istrator checks the /etc/passwd file and reviews the following output: Joe:x:1002:1002::/home/Joe:/sbin/nologin Which of the following should the administrator do to fix this issue?",
    "choices": [
      "Re move the x entry in the /etc/ passwd file for Joe",
      "Ch ange the /sbin/nologin entry to /bin/ sh for Joe",
      "Issue the usermod -l Joe comman d",
      "Ensure that a corresponding entry is in the /etc/shadow file for Joe Answ er: B Explanation: The /sbin/nologin shell is used to prevent a user from logging in. To allow Joe to log in, the administrator should change his shell to /bin/sh, which is a st andard login shell in Linux. This change will allow Joe to access the system normally."
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    "id": 336,
    "type": "mc",
    "question": "A systems administrator is configuring a Linux system so that n etwork traffic from the internal network 172.17.0.0 going out through the eth0 interface would a ppear as if it was sent directly from this interface. Which of the following commands will accom plish this task?",
    "choices": [
      "ipta bles -A POSTROUTING -s 172.17.0.0 -o eth0 -j MASQUERAD E",
      "fire walld -A OUTPUT -s 172.17.0.0 -o eth0 -j DIRECT",
      "n mcli snasq-traffic eth0 -s 172.17.0.0 -j MASQUE RADE",
      "ifconfig --nat eth0 -s 172.17.0.0 -j DIR ECT Answ er: A Explanation: The command iptables -A POSTROUTING -s 172.17.0.0 -o eth0 -j MA SQUERADE configures Network Address Translation (NAT) using iptables. This ensures that traffic from the internal network (172.17.0.0) going out through the eth0 interface will appear t o be originating from the eth0 IP address. This is commonly used for internet-sharing or NAT setu ps in Linux."
    ],
    "answer": "A",
    "explanation": ""
  },
  ...
  },
  {
    "id": 340,
    "type": "mc",
    "question": "While troubleshooting server issues, a Linux systems administra tor obtains the following output: yaml total used free shared buff/cache available Mem: 3736 3538 33 2 48 25 Swap: 2047 1824 223 Which of the following best describes the state of the system?",
    "choices": [
      "The s ystem has consumed the system memory and swap sp ace",
      "The system has enough free memory s pace",
      "The system has swap disabled",
      "The s ystem has allocated enough buffer space Answ er: A Explanation: The output shows that the system is running low on available me mory and has heavily utilized swap space (1824 MB out of 2047 MB). This indicates that both RAM and swap are being exhausted, which can lead to performance degradation. The system is likely exper iencing memory pressure and may require additional RAM or optimizations to reduce memory consum ption."
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    "id": 341,
    "type": "mc",
    "question": "An administrator made a change to a systems network configurati on. Which of the following best represents what the administrator should do to have the new con figuration take effect?",
    "choices": [
      "s ystemctl restart netwo rkd",
      "s ystemctl enable netw orkd",
      "sy stemctl status networkd",
      "s ystemctl isolate netwo rkd Answ er: A Explanation: The command systemctl restart networkd is used to apply new net work configurations by restarting the network service. This ensures that any changes made to netw ork settings (such as IP addresses, routes, or DNS settings) take effect without requiring a system reboot."
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    "id": 342,
    "type": "mc",
    "question": "A Linux administrator logs in to a system and identifies that an important backup has been started. The backup process is consuming a considerable amount of CPU time but needs to continue. Which of the following should the administrator use to reduce the impact this process has on other services?",
    "choices": [
      "renice -n 15 -p <back up pid>",
      "nic e -n 15 -p <backu p pid>",
      "renice -n -15 -p < back up pid>",
      "nice -n -15 -p <backu p pid> Answ er: A Explanation: The renice command changes the scheduling priority of a running process. By using renice -n 15, the administrator lowers the priority of the backup process, making it less CPU-intensive and reducing its impact on other system services. A positive niceness value (e.g ., 15) lowers the priority, allowing other processes to receive more CPU time."
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    ...
    "choices": [
      ...
      "snap find",
      "snap install",
      "snap tr y Answ er: A Explanation: The snap list command lists all installed snap packages on the system. It provides details such as the package name, version, and installation status. This command is useful for verifying whether a specific package was installed using the snap package manager."
    ],
    "answer": "A",
    "explanation": ""
  },
  {
    ...
    "choices": [
      ...
      "bash for server in $(cat /home/user/serverslist) do ssh -i \"for user i n $(cat /home/user/userslist) do sudo useradd $user done; exit\" done",
      "bash for server in $(cat /home/user/serverslist) do scp /home/user/userslist ssh -i \"for user in $(cat /tmp/userslist) do sudo useradd $user done; exit\" done",
      "bash ssh /home/user/serve rslist) \"sudo useradd $(cat /hom e/user/userslist); exit\""
    ],
    "answer": "B",
  ...
  },
  {
    "id": 350,
    "type": "mc",
    "question": "Linux system users report that over the past few months, a serv er has stopped responding at least twice each day. Every time this happens, the users have to rest art the server to recover it. A systems administrator diagnosing the issue collects the following outpu ts: csharp Output 1 [Tue Aug 31 16:36:42 2021] oom-killer invoked by java: gfp_mask=0x280da2 order=0, oom_score_adj=0 [Tue Aug 31 16:36:42 2021] java cpuset=/ mems_allowed=0 makefile Output 2 []# free -m total used free shared buff/cache available Mem: 15819 15026 174 92 619 134 Swap: 0 0 0 Which of the following is the best permanent resolution for the system?",
    "choices": [
  ...
  },
  {
    "id": 7,
    "type": "dragdrop",
    "question": "DRAG & DROP — Create a script that finds rotated log files in /var/log (no recursion) and compresses them.",
    "context": "<p>Add the correct lines in order to <code>/tmp/script.sh</code> to compress rotated logs under <code>/var/log</code> without recursion.</p><pre># /tmp/script.sh\n</pre>",
    "parts": [
      "#!/bin/bash",
      "set -euo pipefail",
      "find /var/log -maxdepth 1 -type f -name '*.log.*' -not -name '*.gz' -exec gzip {} \\;"
    ],
    "distractors": [
      "find /var/log -type d -name '*.gz' -delete",
      "tar -czf /var/log/archive.tar.gz /var/log",
      "gzip -d /var/log/*.gz"
    ],
    "solution_order": [
      0,
    ...
    "explanation": "Use a non-recursive find (-maxdepth 1) to match rotated logs and run gzip on each file.",
    "answer": "A"
  }
];
